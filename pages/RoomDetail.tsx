...
import {
  Archive, ArrowLeft, CheckCircle, Edit2, FileText, Mail, Phone,
  Plus, Search, User, UserRoundX, Users, Wrench, XCircle, Zap,
} from 'lucide-react';
...
import { Contract, Customer, EquipmentStatus, Room, ServiceRecord } from '../types';
...
// ──────────────────────────────────────────────────────────────
// Quick Room Search Dropdown
// ──────────────────────────────────────────────────────────────
function QuickRoomSearch({ currentRoomId }: { currentRoomId: string }) {
  const { rooms, buildings } = useApp();
  ...
  const filtered = rooms
    .filter(r => {
      ...
      const bldName = buildings.find(b => b.id === r.buildingId)?.name || '';
      return (
  ...
  // Group by building
  const grouped = filtered.reduce<Map<string, Room[]>>((acc, room) => {
    const key = room.buildingId;
    if (!acc.has(key)) acc.set(key, []);
    acc.get(key)!.push(room);
    return acc;
  }, new Map());

  const statusDot = (status: Room['status']) => {
    if (status === 'Đang ở') return 'bg-rose-400';
    if (status === 'Trống') return 'bg-emerald-400';
    return 'bg-amber-400';
  };
  ...
  return (
    <div ref={containerRef} className="relative">
      <button
        onClick={() => setOpen(o => !o)}
        className="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-sm text-gray-600 shadow-sm transition hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700"
      ...
      {open && (
        <div className="absolute left-0 top-full z-50 mt-1.5 w-80 rounded-xl border border-gray-200 bg-white shadow-2xl dark:border-gray-700 dark:bg-gray-900">
          {/* Search input */}
          ...
          <div className="max-h-72 overflow-y-auto p-1">
            {grouped.size === 0 ? (
              <p className="p-3 text-center text-sm text-gray-400">Không tìm thấy phòng nào</p>
            ) : (
              Array.from(grouped.entries()).map(([bldId, bldRooms]) => {
                const bld = buildings.find(b => b.id === bldId);
                return (
                  <div key={bldId} className="mb-1">
                    <div className="sticky top-0 bg-white px-3 py-1 text-xs font-semibold uppercase tracking-wider text-gray-400 dark:bg-gray-900">
                      {bld?.name || bldId}
                    </div>
                    {bldRooms.map(room => (
                      <button
                        ...
                        className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left text-sm transition hover:bg-gray-50 dark:hover:bg-gray-800"
                      >
                        <span className={`h-2 w-2 flex-shrink-0 rounded-full ${statusDot(room.status)}`} />
                        <span className="flex-1 font-medium text-gray-900 dark:text-white">{room.name}</span>
                        <span className="text-xs text-gray-400">{formatCurrency(room.price)}</span>
                      </button>
                    ))}
                  </div>
                );
          ...
          <div className="border-t border-gray-100 px-3 py-1.5 text-xs text-gray-400 dark:border-gray-800">
            {rooms.length - 1} phòng khác · Enter để xem tất cả
          </div>
        </div>
      )}
    </div>
...
function EditRoomModal({ room, onClose, onSave }: EditRoomModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) return;
    onSave({ ...room, name: name.trim(), price, floor });
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div className="w-full max-w-sm rounded-xl border border-gray-200 bg-white p-6 shadow-xl dark:border-gray-800 dark:bg-gray-900">
        <div className="mb-5 flex items-center justify-between">
          <h3 className="text-lg font-bold text-gray-900 dark:text-white">Sửa thông tin phòng</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 outline-none focus:ring-2 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white"
            />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Giá niêm yết (VNĐ)</label>
              <input
                required
                type="number"
                min={0}
          ...
          <div className="flex gap-3 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 rounded-lg bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700"
            >
              Hủy
            </button>
            <button
...
// ──────────────────────────────────────────────────────────────
// Main RoomDetail Page
// ──────────────────────────────────────────────────────────────
export default function RoomDetail() {
  const { id: roomId = '' } = useParams();
  const {
    rooms, contracts, customers, payments, serviceRecords, addServiceRecord,
    equipment, buildings, currentUser, updateEquipment, terminateContract,
    updateCustomer, generateRoomBill, updateRoom,
  } = useApp();
  ...
  const handleAddService = (event: React.FormEvent) => {
    ...
    const calcElectric = isElectricFixed ? electricPrice : newService.electricUsage * electricPrice;
    const calcWater = isWaterFixed ? waterPrice : newService.waterUsage * waterPrice;
    const total = calcElectric + calcWater + newService.internetCost + newService.otherCost;
  ...
  const getStatusColor = (status: EquipmentStatus) => {
    switch (status) {
      case 'Tốt': return 'text-green-500';
      case 'Hỏng': return 'text-red-500';
      case 'Đang sửa': return 'text-yellow-500';
      case 'Thanh lý': return 'text-gray-500';
  ...
  const getStatusIcon = (status: EquipmentStatus) => {
    switch (status) {
      case 'Tốt': return <CheckCircle className="w-4 h-4" />;
      case 'Hỏng': return <XCircle className="w-4 h-4" />;
      case 'Đang sửa': return <Wrench className="w-4 h-4" />;
      case 'Thanh lý': return <Archive className="w-4 h-4" />;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <Link
          to="/app/rooms"
          className="inline-flex items-center gap-2 text-gray-400 transition hover:text-gray-900 dark:hover:text-white"
        >
          <ArrowLeft className="w-4 h-4" /> Quay lại danh sách phòng
        </Link>

        {/* Quick Room Search */}
        <QuickRoomSearch currentRoomId={room.id} />
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            ...
            <div className="mb-2 flex items-start gap-2">
              <h1 className="flex-1 text-3xl font-bold text-gray-900 dark:text-white">{room.name}</h1>
              <button
                type="button"
                onClick={() => setIsEditRoomOpen(true)}
                title="Sửa thông tin phòng"
                className="mt-1 rounded-lg p-1.5 text-gray-400 transition hover:bg-gray-100 hover:text-gray-700 dark:hover:bg-gray-800 dark:hover:text-gray-200"
              >
            ...
            <div className="mb-4 flex flex-wrap items-center gap-2">
              <span
                className={`rounded-full border px-3 py-1 text-xs font-medium ${
                  room.status === 'Đang ở'
                    ? 'border-red-200 bg-red-50 text-red-600 dark:border-red-500/50 dark:bg-red-500/20 dark:text-red-500'
                    : room.status === 'Trống'
                    ? 'border-green-200 bg-green-50 text-green-600 dark:border-green-500/50 dark:bg-green-500/20 dark:text-green-500'
              ...
              {activeBuilding && (
                <span className="text-xs text-gray-400">
                  📍 {activeBuilding.name} · Tầng {room.floor}
                </span>
              )}
            </div>
            ...
            <div className={`mt-4 rounded-xl border px-4 py-3 text-sm font-medium ${getBillStatusTone(currentRoomBill?.status || 'missing')}`}>
              {currentRoomBill
                ? `Bill ${currentPeriod}: ${
                    currentRoomBill.status === 'Đã đóng'
                      ? 'Đã đóng tiền tháng'
                      : currentRoomBill.status === 'Quá hạn'
                      ? 'Đang quá hạn thanh toán'
                      : 'Chưa đóng tiền tháng'
                  }`
            ...
            {roomResidents.length > 0 && (
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => {
                    const period = prompt('Nhập kỳ thu (VD: 04/2026)', currentPeriod);
                    if (period) generateRoomBill(roomId, period);
                  }}
                  className="flex items-center gap-2 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-sm text-blue-700 shadow-sm transition-colors hover:bg-blue-100 dark:border-blue-800 dark:bg-blue-950/30 dark:text-blue-300 dark:hover:bg-blue-900/50"
                >
                  <FileText className="w-4 h-4" /> Tạo bill thủ công
                </button>
                <button
                  type="button"
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div className="rounded-lg border border-gray-200 bg-white p-3 shadow-sm dark:border-gray-800 dark:bg-gray-900/50">
              <div className="mb-1 text-xs text-gray-500 dark:text-gray-400">
                {isElectricFixed ? 'Tiền điện (Khoán)' : (activeContracts.length > 1 ? 'Đơn giá Điện (HĐ gần nhất)' : 'Đơn giá Điện')}
              </div>
              <div className="font-bold text-yellow-600 dark:text-yellow-500">
                {formatCurrency(electricPrice)} {!isElectricFixed && '/kWh'}
              </div>
            </div>
            <div className="rounded-lg border border-gray-200 bg-white p-3 shadow-sm dark:border-gray-800 dark:bg-gray-900/50">
              <div className="mb-1 text-xs text-gray-500 dark:text-gray-400">
                {isWaterFixed ? 'Tiền nước (Khoán)' : (activeContracts.length > 1 ? 'Đơn giá Nước (HĐ gần nhất)' : 'Đơn giá Nước')}
              </div>
              <div className="font-bold text-blue-600 dark:text-blue-500">
                {formatCurrency(waterPrice)} {!isWaterFixed && '/khối'}
              </div>
            </div>
            <div className="rounded-lg border border-gray-200 bg-white p-3 shadow-sm dark:border-gray-800 dark:bg-gray-900/50">
              <div className="mb-1 text-xs text-gray-500 dark:text-gray-400">
              ...
              <div className="font-bold text-indigo-600 dark:text-indigo-400">{formatCurrency(internetPrice)} /phòng</div>
            </div>
          </div>
              ...
              <table className="w-full text-left">
                <thead className="border-b border-gray-200 bg-gray-50 text-xs font-semibold uppercase text-gray-500 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400">
                  <tr>
                    ...
                    <th className="p-4 text-right">Internet</th>
                  ...
                  {roomServices.map(service => (
                    <tr key={service.id} className="transition-colors hover:bg-gray-50 dark:hover:bg-gray-800/50">
                      ...
                      <td className="p-4 text-center">
                        ...
                        <div className="text-xs text-gray-500">{formatCurrency(isElectricFixed ? electricPrice : service.electricUsage * electricPrice)}</div>
                      </td>
                      <td className="p-4 text-center">
                        <div className="text-sm text-gray-900 dark:text-white">{service.waterUsage}</div>
                        <div className="text-xs text-gray-500">{formatCurrency(isWaterFixed ? waterPrice : service.waterUsage * waterPrice)}</div>
                      </td>
      ...
      {/* ── Modals ── */}
      {isEditRoomOpen && (
        <EditRoomModal room={room} onClose={() => setIsEditRoomOpen(false)} onSave={handleSaveRoom} />
      )}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">
                    Số điện {isElectricFixed ? '(Khoán — bỏ qua)' : '(kWh)'}
                  </label>
                  <input
                    required
                    type="number"
                    disabled={isElectricFixed}
                    className="w-full rounded-lg border border-gray-300 bg-white p-2 text-gray-900 outline-none focus:ring-2 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white disabled:opacity-50"
                    value={newService.electricUsage}
                    onChange={event => setNewService({ ...newService, electricUsage: Number(event.target.value) })}
                  />
                  {isElectricFixed && (
                    <p className="mt-1 text-xs text-amber-600 dark:text-amber-400">Điện khoán: {formatCurrency(electricPrice)}/tháng</p>
                  )}
                </div>
                <div>
                  <label className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">
                    Số nước {isWaterFixed ? '(Khoán — bỏ qua)' : '(m³)'}
                  </label>
                  <input
                    required
                    type="number"
                    disabled={isWaterFixed}
                    className="w-full rounded-lg border border-gray-300 bg-white p-2 text-gray-900 outline-none focus:ring-2 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white disabled:opacity-50"
                    value={newService.waterUsage}
                    onChange={event => setNewService({ ...newService, waterUsage: Number(event.target.value) })}
                  />
                  {isWaterFixed && (
                    <p className="mt-1 text-xs text-amber-600 dark:text-amber-400">Nước khoán: {formatCurrency(waterPrice)}/tháng</p>
                  )}
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
              ...
              <div className="mt-2 rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-800">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-bold text-green-600 dark:text-green-400">
                    {formatCurrency(
                      (isElectricFixed ? electricPrice : newService.electricUsage * electricPrice) +
                      (isWaterFixed ? waterPrice : newService.waterUsage * waterPrice) +
      ...
      {residentToTerminate && (
        <TerminateContractModal
          ...
          outstandingPayments={payments.filter(
            payment => payment.contractId === residentToTerminate.contract.id && payment.status !== 'Đã đóng',
          )}
      ...
      {isQuickContractOpen && (
        <QuickContractWizard
          ...
          onClose={() => setIsQuickContractOpen(false)}
        />
      )}
    </div>
  );
