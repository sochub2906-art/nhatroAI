                    ...
                    <div className="max-h-[94vh] w-full max-w-5xl overflow-y-auto rounded-[2rem] border border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-900">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6 p-5">
                            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                                ...
                                <input type="date" lang="vi-VN" required value={draft.purchaseDate} onChange={event => setDraft(prev => ({ ...prev, purchaseDate: event.target.value }))} className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none transition focus:border-blue-400 focus:ring-2 focus:ring-blue-100 dark:border-slate-700 dark:bg-slate-950 dark:text-white" />
                                ...
                                <input type="date" lang="vi-VN" value={draft.lastValuationDate} onChange={event => setDraft(prev => ({ ...prev, lastValuationDate: event.target.value }))} className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none transition focus:border-blue-400 focus:ring-2 focus:ring-blue-100 dark:border-slate-700 dark:bg-slate-950 dark:text-white" />
                            </div>
                                    ...
                                    draft.maintenanceHistory.map((entry, index) => (
                                        <div key={entry.id} className="rounded-[1.5rem] border border-slate-200 p-4 dark:border-slate-700">
                                            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-[0.9fr_0.9fr_0.9fr_1fr_auto]">
                                                ...
                                                <input type="date" lang="vi-VN" value={entry.date} onChange={event => setMaintenanceField(index, 'date', event.target.value)} className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none transition focus:border-blue-400 focus:ring-2 focus:ring-blue-100 dark:border-slate-700 dark:bg-slate-950 dark:text-white" />
                                                ...
                                                <button type="button" onClick={() => setDraft(prev => ({ ...prev, maintenanceHistory: prev.maintenanceHistory.filter((_, currentIndex) => currentIndex !== index) }))} className="inline-flex h-12 items-center justify-center rounded-2xl border border-rose-200 px-4 text-rose-600 transition hover:bg-rose-50 dark:border-rose-900/30 dark:text-rose-300 dark:hover:bg-rose-950/20"><Trash2 className="h-4 w-4" /></button>
                                            </div>
                                            <div className="mt-2 text-xs text-slate-500 dark:text-slate-400">{MAINTENANCE_LABELS[entry.type]}</div>
                                            <textarea rows={3} value={entry.note} onChange={event => setMaintenanceField(index, 'note', event.target.value)} placeholder="Ghi chú sửa chữa, thay thế, đơn vị thực hiện..." className="mt-4 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none transition focus:border-blue-400 focus:ring-2 focus:ring-blue-100 dark:border-slate-700 dark:bg-slate-950 dark:text-white" />
                                        </div>
                                    ))
                                )}
                            </div>
