...
function openHtmlWindow(html: string, autoPrint = false): void {
    const popup = window.open('', '_blank', 'noopener,noreferrer,width=1080,height=900');
    if (!popup) {
        alert('Trình duyệt đang chặn cửa sổ in phiếu. Vui lòng cho phép popup và thử lại.');
        return;
    }
...
export default function BillModal({ bill, onClose }: BillModalProps) {
    ...
    const copyBillText = async (showAlert = true) => {
        try {
    ...
    const handleOpenChannel = async (channel: 'zalo' | 'messenger') => {
        const copied = await copyBillText(false);
        let url = '';
        ...
        showToast(
            copied
                ? `Đã sao chép bill. Tiếp tục dán vào ${channel === 'zalo' ? 'Zalo' : 'Messenger'} để gửi.`
                : `Không tự động sao chép được. Mở ${channel === 'zalo' ? 'Zalo' : 'Messenger'} rồi gửi thủ công.`
        );
    };
    ...
    const handleDownload = () => {
        ...
        anchor.href = url;
        anchor.download = `phieu-thu-${bill.roomId}-${bill.period.replace(/[^0-9]+/g, '-')}.html`;
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
            <div className="flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-950">
                <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50 px-6 py-5 dark:border-slate-800 dark:bg-slate-900/80">
                    ...
                    <button onClick={onClose} className="rounded-2xl p-2 text-slate-500 transition hover:bg-slate-200 dark:hover:bg-slate-800">
                        <X className="h-5 w-5" />
                    </button>
                </div>
                        ...
                        <div className="space-y-6">
                            <div className="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                                ...
                                <div className="mt-5 grid gap-4 md:grid-cols-2">
                                    ...
                                    <div className="rounded-2xl bg-slate-50 px-4 py-4 dark:bg-slate-800/80">
                                        <div className="text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">Tình trạng bill</div>
                                        <div className="mt-2 text-sm font-medium text-slate-900 dark:text-white">{bill.status}</div>
                                        <div className="mt-2 text-xs text-slate-500 dark:text-slate-400">
                                            {bill.items.length} khoản thu • {bill.customers.length} khách trong phòng
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                                <div className="mb-4 flex items-center justify-between gap-4">
                                    <div>
                                        ...
                                        <p className="text-sm text-slate-500 dark:text-slate-400">Khoản dùng chung theo phòng được gộp lại để bill không bị lặp dòng.</p>
                                    </div>
                                            ...
                                            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                                                <div className="space-y-1">
                                                    ...
                                                    <div className="text-sm text-slate-500 dark:text-slate-400">
                                                        {item.scope === 'room'
                                                            ? 'Dùng chung theo phòng'
                                                            : item.customerNames.join(', ') || 'Theo hợp đồng'}
                                                    </div>
                        ...
                        <div className="space-y-6">
                            <div className="rounded-[1.75rem] border border-blue-200 bg-blue-50 p-5 shadow-sm dark:border-blue-900/40 dark:bg-blue-950/20">
                                ...
                                <p className="mt-2 text-sm text-blue-700 dark:text-blue-300">
                                    Số tiền trên QR đang lấy theo phần còn phải thu của bill phòng này.
                                </p>
                                ...
                                <div className="mt-4 space-y-2 rounded-2xl bg-white px-4 py-4 text-sm shadow-sm dark:bg-slate-900">
                                    <div className="flex justify-between gap-4">
                                        <span className="text-slate-500 dark:text-slate-400">Ngân hàng</span>
                                        <span className="text-right font-medium text-slate-900 dark:text-white">{bankInfo.bankName || 'Chưa cập nhật'}</span>
                                    </div>
                                    <div className="flex justify-between gap-4">
                                        <span className="text-slate-500 dark:text-slate-400">Số tài khoản</span>
                                        <span className="text-right font-medium text-slate-900 dark:text-white">{bankInfo.accountNumber || 'Chưa cập nhật'}</span>
                                    </div>
                                    <div className="flex justify-between gap-4">
                                        <span className="text-slate-500 dark:text-slate-400">Chủ tài khoản</span>
                                        <span className="text-right font-medium text-slate-900 dark:text-white">{bankInfo.accountName || 'Chưa cập nhật'}</span>
                                    </div>
                                    <div className="border-t border-slate-200 pt-2 dark:border-slate-700">
                                        <div className="text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">Nội dung chuyển khoản</div>
                                        <div className="mt-1 break-all rounded-xl bg-slate-50 px-3 py-2 font-mono text-xs text-slate-700 dark:bg-slate-800 dark:text-slate-200">
                                            {transferContent}
                                        </div>
                                        <div className="mt-2 text-xs text-slate-500 dark:text-slate-400">
                                            Mã bill này được dùng để đối soát tự động khi webhook báo tiền vào.
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                                ...
                                <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
                                    Nội dung phiếu sẽ được sao chép sẵn trước khi mở Zalo hoặc Messenger để gửi nhanh cho khách.
                                </p>
                ...
                <div className="flex flex-col gap-3 border-t border-slate-100 bg-slate-50 px-6 py-5 dark:border-slate-800 dark:bg-slate-900/80 sm:flex-row">
                    <button
                        type="button"
                        onClick={() => openHtmlWindow(documentHtml, true)}
