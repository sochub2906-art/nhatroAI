...
function parseServiceRecord(raw: any): ServiceRecord {
    return {
        id: String(pickRaw(raw, 'ID', 'id') || ''),
        roomId: String(pickRaw(raw, 'MÃ PHÒNG', 'ROOM_ID', 'roomId', 'M?? PH??NG', 'MÃƒ PHÃ’NG') || ''),
        month: String(pickRaw(raw, 'THÁNG', 'PERIOD', 'month', 'TH??NG', 'THÃNG') || ''),
        electricOldReading: Number(pickRaw(raw, 'ĐIỆN_CŨ', 'ELECTRIC_OLD', 'electricOldReading', '?I?N_C?', 'ÄIá»†N_CÅ¨') || 0),
        electricNewReading: Number(pickRaw(raw, 'ĐIỆN_MỚI', 'ELECTRIC_NEW', 'electricNewReading', '?I?N_M?I', 'ÄIá»†N_Má»I') || 0),
        electricUsage: Number(pickRaw(raw, 'ĐIỆN', 'ELECTRIC_USAGE', 'electricUsage', '??I=N', 'ÄIá»†N') || 0),
        waterOldReading: Number(pickRaw(raw, 'NƯỚC_CŨ', 'WATER_OLD', 'waterOldReading', 'N??C_C?', 'NÆ¯á»C_CÅ¨') || 0),
        waterNewReading: Number(pickRaw(raw, 'NƯỚC_MỚI', 'WATER_NEW', 'waterNewReading', 'N??C_M?I', 'NÆ¯á»C_Má»I') || 0),
        waterUsage: Number(pickRaw(raw, 'NƯỚC', 'WATER_USAGE', 'waterUsage', 'N=??C', 'NÆ¯á»C') || 0),
        internetCost: Number(pickRaw(raw, 'INTERNET', 'internetCost') || 0),
        otherCost: Number(pickRaw(raw, 'KHÁC', 'otherCost', 'KH??C', 'KHÃC') || 0),
        totalCost: Number(pickRaw(raw, 'TỔNG', 'totalCost', 'T=NG', 'Tá»NG') || 0),
